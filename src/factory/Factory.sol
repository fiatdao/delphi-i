...
contract Factory {
    ...
    function deployDiscountRateArchitecture(DiscountRateDeployData memory deployData)
        public
        returns (address)
    { 
        // Check if we need to create the Discount Rate Relayer
        address discountRateRelayerAddress = deployData.discountRateRelayerAddress;
        ...
        if(createRelayer){
            ...
            discountRateRelayerAddress = deployCollybusDiscountRateRelayer(deployData.collybusAddress);
        }

        // We check if we have any national aggregators to deploy
        uint notionalAggregatorCount = deployData.notionalData.length;
        for(uint notionalAggIndex = 0; notionalAggIndex < notionalAggregatorCount; notionalAggIndex++ ){
            // Decode each input notional aggregator structure 
            AggregatorData memory aggData = abi.decode(deployData.notionalData[notionalAggIndex], (AggregatorData));

            address agregatorAddress = aggData.aggregatorAddress;
            //Check where we need to create the aggregator
            bool createTheAggregator = createRelayer || (agregatorAddress == address(0));

            // For each Notional aggregator we find we will go though the oracles and create each one
            uint notionalOracleCount = aggData.oracleData.length;
            ...
            for (uint notionalOracleIndex=0; notionalOracleIndex < notionalOracleCount; notionalOracleIndex++){
                NotionalData memory notionalData = abi.decode(aggData.oracleData[notionalOracleIndex], (NotionalData));
                ...
                address oracleAddress = deployOracle(notionalVP,
                ...
                if(createTheAggregator){
                    ...
                    IAggregatorOracle(agregatorAddress).oracleAdd(oracleAddress);
                }
            }

            if(createTheAggregator){
                agregatorAddress = deployAggregator(oracleList,
                aggData.requiredValidValues);

                ICollybusDiscountRateRelayer(discountRateRelayerAddress).oracleAdd(agregatorAddress, aggData.tokenId, aggData.minimumThresholdValue);
            } 
        }

        // We check if we have any element aggregators to deploy
        uint elementAggregatorCount = deployData.elementData.length;
        for(uint elementAggIndex = 0; elementAggIndex < elementAggregatorCount; elementAggIndex++ ){
            // Decode each input aggregator structure 
            AggregatorData memory aggData = abi.decode(deployData.elementData[elementAggIndex], (AggregatorData));
            ...
            uint elementOracleCount = aggData.oracleData.length;
            ...
            for (uint elementOracleIndex=0; elementOracleIndex < elementOracleCount; elementOracleIndex++){
                ElementData memory elementData = abi.decode(aggData.oracleData[elementOracleIndex], (ElementData));
                ...
                address oracleAddress = deployOracle(elementVP,
